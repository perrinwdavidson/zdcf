...
c                                                                             c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine zdcf(ta,a,erra,mbins,maxlag,mpnts,minbin,rslutn,
     >                tdcf,dcf)
      implicit none
      integer mpnts,mbins,minbin
      real maxlag,rslutn
c f2py intent(in) :: mpnts, mbins, minbin, maxlag, rslutn
c*****************************************************************************c
c                                                                             c
...
c                                                                             c
c      parameter (mpnts = 1550, mbins = 500)
c*****************************************************************************c
      integer i,n
...
c f2py intent(in) :: ta, a, erra
c f2py intent(out) :: tdcf, dcf
...
c     
c      print *,'ZDCF V1.2 begins.'
c      print *,'Auto-correlation or cross-correlation? (1/2):'
c      read *,inpi
c      autocf = (inpi .eq. 1)
      autocf = 1
c     
c      print *,'Enter output files prefix:'
c      read '(a72)',prefix
c      do i = 1,len(prefix)
c         if (prefix(i:i) .eq. ' ') then
c            lprfix = i-1
c            goto 200
c         endif
c      enddo
c200   continue
c     
c      print *,'Uniform sampling of light curve? (y/n):'
c      read '(a1)',inpc
c      unsmpl = (inpc .eq. 'y' .or. inpc .eq. 'Y')
      unsmpl = .false.
c      print *,'Enter minimal number of points per bin (0 for default):'
c      read *,minpts
c if minpts = 0, use default
c      if (minpts .le. 0) minpts = enough
      minpts = enough
c     
c      print *,'Omit zero-lag points? (y/n):'
c      read '(a1)',inpc
c      no0lag = (inpc .eq. 'y' .or. inpc .eq. 'Y') 
      no0lag = .false.
c     
c      print *,'How many Monte Carlo runs for error estimation?'
c      read *,ncarlo
c      if (ncarlo.le.1) ncarlo = 0
      ncarlo = 0
c     
c Reading the observed data
c     
c      print *,'Enter name of 1st light curve file:'
c      read '(a72)',infil1
c      if (.not. autocf) then
c         print *,'Enter name of 2nd light curve file:'
c         read '(a72)',infil2
c      endif
c      call redobs(na,nb,ta,tb,a,b,erra,errb,mpnts,
c     >            autocf,infil1,infil2)
      na = mpnts
      nb = na
      b = a
      tb = ta
...
c     
c      print *
c      outfil = prefix(:lprfix)//'.lc1'
c      open (unit=11,file=outfil,status='UNKNOWN',iostat=oerr,err=901)
c      write (11,'(1p,3(1x,e12.5))')
c     >   (ta(i),a(i),erra(i),i=1,na)
c      close (unit=11,status='KEEP',iostat=cerr,err=902)
c      print *,outfil(:lprfix+5),' written (contains ',na,' points) ...'
c     
c      outfil = prefix(:lprfix)//'.lc2'
c      open (unit=12,file=outfil,status='UNKNOWN',iostat=oerr,err=901)
c      write (12,'(1p,3(1x,e12.5))')
c     >   (tb(i),b(i),errb(i),i=1,nb)
c      close (unit=12,status='KEEP',iostat=cerr,err=902)
c      print *,outfil(:lprfix+5),' written (contains ',nb,' points) ...'
c     
c      print *
c      print *,('=',i=1,75)
c      print *,'ZDCF PARAMETERS:'
c      print *,'Autocorrelation?  ',autocf
c      print *,'Uniform sampling? ',unsmpl
c      print *,'Omit zero lags?   ',no0lag
c      print *,'Minimal # in bin: ',minpts
c      print *,'# of MonteCarlo:  ',ncarlo 
c      print *,'MonteCarlo seed:  ',seed 
c      print *,na,' points in ',infil1(1:40)
c      if (.not. autocf) print *,nb,' points in ',infil2(1:40)
c      print *,('=',i=1,75)
c      print *
c     
...
c     
c      i = rand(seed)
c      if (ncarlo .gt. 1) then
c         do i = 1,mbins
c            avz(i) = 0.0
c         enddo
c         nwdata = .true.
c         do i = 1,ncarlo
c            call simerr(a,acarlo,erra,na)
c            if (autocf) then
c               call clcdcf(ta,acarlo,erra,na,
cc                          i  i      i    i
c     >                     tb,acarlo,errb,nb,
cc                          i  i      i    i
c     >             autocf,no0lag,unsmpl,used,unused,nwdata,
cc                  i      i      i      o    o      i/o    
c     >             minpts,tdcf,sigtm,sigtp,dcf,sigdcm,sigdcp,
cc                  i/o    o    o     o     o   o      o    
c     >             inbin,minbin,nbins,mbins,
cc                  i    i      i/o   i
c     >             wta,wtb,wa,wb,werra,werrb,wai,wbj,wuseda,wusedb,
cc                  w   w   w  w  w     w     w   w   w      w
c     >             vtau)
cc                  o
c            else
c               call simerr(b,bcarlo,errb,nb)
cc     
cc calculating the discrete correlation function for Monte Carlo error approx.
cc     
c               call clcdcf(ta,acarlo,erra,na,
cc                          i  i i    i
c     >                     tb,bcarlo,errb,nb,
...
cc            if (i .eq. 1) then
cc               print *
cc               print *,nbins,' bins actually used, ',unused,
cc     >              ' inter-dependent pairs discarded.'
cc            endif
cc     
cc The summing and averaging is done in z-space.
cc     
c            do n = 1,nbins
c               r = dcf(n)
c               if (r .lt. -1.0+eps) then
c                  r = -1.0+eps
c               else if (r .gt.  1.0-eps) then
c                  r =  1.0-eps
c               endif   
c               avz(n) = avz(n)+log((1.+r)/(1.-r))/2.
c            enddo
c         enddo
c         do i = 1,nbins
c            z = avz(i)/(ncarlo+0.)
...
c            sigz = fishs(r,n)
c            expz = fishe(r,n)
c            sigdcm(i) = r-tanh(expz-sigz)
c            sigdcp(i) = tanh(expz+sigz)-r
c         enddo
c      else
c     
...
c                    i/o    o    o     o     o   o      o      
     >               inbin,minbin,nbins,mbins,mpnts,
c                    o     i      i/o   i     i
...
c               o
c         print *
c         print *,nbins,' bins actually used, ',unused,
c     >           ' inter-dependent pairs discarded.'
c      endif
c     
c printing the results: tdcf, dcf, sigdcf
c     
c      outfil = prefix(:lprfix)//'.dcf'
c      open (unit=13,file=outfil,status='UNKNOWN',iostat=oerr,err=901)
c      print *
c      print '(1p,6(1x,a10),'' ('',a4,'')'')',
c     >      ' tau      ','-sig(tau) ','+sig(tau) ',' dcf      ',
c     >      ' -err(dcf)',' +err(dcf)','#bin'
c      print *
c      do i = 1,nbins
c         print '(1p,6(1x,e10.3),'' ('',i4,'')'')',
c     >         tdcf(i),sigtm(i),sigtp(i),
c     >         dcf(i),sigdcm(i),sigdcp(i),inbin(i)
c         write (13,'(1p,6(1x,e10.3),1x,i4)') 
c     >         tdcf(i),sigtm(i),sigtp(i),
c     >         dcf(i),sigdcm(i),sigdcp(i),inbin(i)
c      enddo
c      close (unit=13,status='KEEP',iostat=cerr,err=902)
c      print *
c      print *,outfil(:lprfix+4),' written...'
c     
c      print *,'ZDCF ended.'
c      stop
c 901  print *,'ERROR ON OPENING FILE - ERRCOD=',oerr
c      stop
c 902  print *,'ERROR ON CLOSING FILE - ERRCOD=',cerr
c      stop
      end      
c//////////////////////////////////////////////////////////////////////////////
c     
...
c     
     ...
     >                  tdcf,sigtm,sigtp,
     >                  wai,wbj,inbin,nbins,mbins,minbin,unsmpl,mpnts,
     >                  maxlag,vtau,rslutn)
      implicit none
c*****************************************************************************c
      real rslutn
c      parameter (rslutn=0.001)
c      parameter (rslutn=0.0005)
c*****************************************************************************c
...
c     
      if (frstym) then
         frstym = .false.
c         print 
c     >      '(''Binning with minimum of '',i3,'//
c     >      ''' points per bin and resolution of '',1p,g8.2,'' .'')',
c     >      minpts,rslutn
      endif
      np = 0
      if (autocf) then
...
c                       i       w   i
      ...
      integer ibin
      integer na,nb,nbins,minbin,mbins,unused,minpts,mpnts
      real ta(na),tb(nb)
      real a(na),b(nb)
